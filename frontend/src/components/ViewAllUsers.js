...
export default function ViewAllUsers() {
  ...
  useEffect(() => {
    ...
    async function getUsers() {
      try {
        ...
        ); // Adjust the endpoint based on your API route
  ...
  const filteredReports = users.filter((rep) => {
    return (
      rep.firstname.toLowerCase().includes(filter.toLowerCase()) &
      rep.role.toLowerCase().includes(filterRole.toLowerCase())
    );
  });
        ...
        <div class="flex flex-wrap justify-between space-y-4 md:space-y-0">
          <div class="w-full md:w-1/4">
            <div class="form-group">
              <label for="roleFilter" class="text-white">
                Role Filter
              </label>
              <select
                class="form-control"
                id="roleFilter"
                onChange={filterRoleOp}
              >
              ...
              <tr key={user._id}>
                ...
                <td class="space-x-2">
                  <Link to={"/updateUsers/" + user._id}>
                    <button class="btn btn-sm text-white bg-blue-500">
                      Update
                    </button>
                  </Link>
